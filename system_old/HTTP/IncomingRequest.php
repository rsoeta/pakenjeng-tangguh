...
class IncomingRequest extends Request
{
	...
	public function __construct($config, URI $uri = null, $body = 'php://input', UserAgent $userAgent = null)
	{
		...
		$this->detectURI($config->getUri()Protocol, $config->baseURL);
	...
	public function setPath(string $path, App $config = null)
	{
		...
		if ($baseURL !== '')
		{
			$this->getUri()->setScheme(parse_url($baseURL, PHP_URL_SCHEME));
			$this->getUri()->setHost(parse_url($baseURL, PHP_URL_HOST));
			$this->getUri()->setPort(parse_url($baseURL, PHP_URL_PORT));

			// Ensure we have any query vars
			$this->getUri()->setQuery($_SERVER['QUERY_STRING'] ?? '');

			// Check if the baseURL scheme needs to be coerced into its secure version
			if ($config->forceGlobalSecureRequests && $this->getUri()->getScheme() === 'http')
			{
				$this->getUri()->setScheme('https');
			}
		}
	...
	public function getPath(): string
	{
		...
		{
			$this->detectPath($this->config->getUri()Protocol);
		}
