    ...
    public function tabel_data()
    {
        ...
        foreach ($listing as $key) {
            ...
            if (session()->get('role_id') <= 2) {
                $row[] = '<a class="btn btn-sm" href="javascript:void(0)" title="View" onclick="edit_person(' . "'" . $key->id . "'" . ')"><i class="fa fa-pencil"></i></a>
                            <button class="btn btn-sm" data-id="' . $key->id . '" data-nama="' . $key->nama . '" id="deleteBtn"><i class="far fa-trash-alt"></i></button>';
            } else {
                $row[] = '<a class="btn btn-sm" href="javascript:void(0)" title="View" onclick="edit_person(' . "'" . $key->id . "'" . ')"><i class="fa fa-pencil"></i></a>';
            }
            $data[] = $row;
    ...
    public function importExcel()
    {
        ...
        if ($this->request->getPost(['submit'])) {
            ...
            foreach ($data as $x => $row) {
                ...
                // $cekId = $db->table('dtks_pbi_jkn')->getWhere(['nik' => $nik])->getResult();
                ...
                $simpandata = [
                    'desa_kode' => $desa_kode, 'alamat' => $alamat, 'rw' => $rw, 'nama' => $nama, 'nik' => $nik, 'noka' => $noka, 'tgllhr' => $tgllhr, 'created_by' => $created_by, 'created_at' => $created_at
                ];
                ...
                if (count($cekId) < 1) {
                    $db->table('dtks_pbi_jkn')->insert($simpandata);
                    session()->setFlashdata('message', '<b>Import file, Berhasil!</b>');
                    // $db->query("INSERT IGNORE INTO dtks_pbi_jkn (desa_kode, alamat, rw, nama, nik, noka, tgllhr, created_by, created_at)
                    //             VALUES(" . $desa_kode, $alamat, $rw, $nama, $nik, $noka, $tgllhr, $created_by, $created_at . ")");
                } else {
                    session()->setFlashdata('message', '<b style="color:red">Data Gagal di Import, ada NIK yang sama</b>');
                }
            }
            return redirect()->to('verivalipbi');
        }
